...
interface GlossaryPageProps {
  initialGlossaryTerms?: any[];
}

const GlossaryPage: React.FC<GlossaryPageProps> = ({ initialGlossaryTerms = [] }) => {
  const { t } = useTranslation();
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedCategory, setSelectedCategory] = useState('all');
  const [glossaryTerms] = useState(initialGlossaryTerms);

  // Sample glossary data fallback
  const sampleGlossaryTerms = [
    {
      term: '气 (Qi)',
      category: 'basic',
  ...
  const terms = glossaryTerms.length > 0 ? glossaryTerms : sampleGlossaryTerms;
