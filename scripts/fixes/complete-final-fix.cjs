...
// 📅 Marked obsolete on: 2026-03-10T18:57:28.268Z
// 🔄 Purpose: Internationalization fixes (completed)
// 
const fs = require('fs');
const path = require('path');

console.log('🚀 完成最终修复所有剩余问题...\n');

// 最完整的翻译映射
const completeTranslationMap = {
  // 甲乙经剩余问题
...
// 完成修复特定书籍
function completeFixBookData(bookId, bookName) {
    ...
    try {
      ...
      if (chapterData.sections) {
        chapterData.sections.forEach((section, index) => {
          // 修复现代解读
          if (section.interpretation && completeTranslationMap[section.interpretation]) {
            const originalInterpretation = section.interpretation;
            section.interpretation = completeTranslationMap[section.interpretation];
            hasChanges = true;
          ...
          // 修复白话译文
          if (section.translation && completeTranslationMap[section.translation]) {
            const originalTranslation = section.translation;
            section.translation = completeTranslationMap[section.translation];
            hasChanges = true;
          ...
          // 修复章节摘要
          if (section.summary && completeTranslationMap[section.summary]) {
            const originalSummary = section.summary;
            section.summary = completeTranslationMap[section.summary];
            hasChanges = true;
          ...
          if (section.keyConcepts) {
            section.keyConcepts.forEach(concept => {
              if (concept.term && completeTranslationMap[concept.term]) {
                const originalTerm = concept.term;
                concept.term = completeTranslationMap[concept.term];
                hasChanges = true;
...
books.forEach(book => {
  console.log(`\n📚 完成修复 ${book.name}：`);
  const fixes = completeFixBookData(book.id, book.name);
  totalFixes += fixes;
});
