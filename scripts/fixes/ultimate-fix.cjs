...
// 📅 Marked obsolete on: 2026-03-10T18:57:28.293Z
// 🔄 Purpose: Internationalization fixes (completed)
// 
const fs = require('fs');
const path = require('path');

console.log('🚀 终极修复所有剩余问题...\n');

// 终极翻译映射
const ultimateTranslationMap = {
  // 甲乙经剩余问题
...
// 终极修复特定书籍
function ultimateFixBookData(bookId, bookName) {
    ...
    try {
      ...
      if (chapterData.sections) {
        chapterData.sections.forEach((section, index) => {
          // 修复现代解读
          if (section.interpretation && ultimateTranslationMap[section.interpretation]) {
            const originalInterpretation = section.interpretation;
            section.interpretation = ultimateTranslationMap[section.interpretation];
            hasChanges = true;
          ...
          // 修复白话译文
          if (section.translation && ultimateTranslationMap[section.translation]) {
            const originalTranslation = section.translation;
            section.translation = ultimateTranslationMap[section.translation];
            hasChanges = true;
          ...
          // 修复章节摘要
          if (section.summary && ultimateTranslationMap[section.summary]) {
            const originalSummary = section.summary;
            section.summary = ultimateTranslationMap[section.summary];
            hasChanges = true;
          ...
          if (section.keyConcepts) {
            section.keyConcepts.forEach(concept => {
              if (concept.term && ultimateTranslationMap[concept.term]) {
                const originalTerm = concept.term;
                concept.term = ultimateTranslationMap[concept.term];
                hasChanges = true;
              ...
              if (concept.category && ultimateTranslationMap[concept.category]) {
                const originalCategory = concept.category;
                concept.category = ultimateTranslationMap[concept.category];
                hasChanges = true;
...
books.forEach(book => {
  console.log(`\n📚 终极修复 ${book.name}：`);
  const fixes = ultimateFixBookData(book.id, book.name);
  totalFixes += fixes;
});
