...
// 📅 Marked obsolete on: 2026-03-10T18:57:28.328Z
// 🔄 Purpose: Internationalization fixes (completed)
// 
const fs = require('fs');
const path = require('path');

console.log('🚀 修复现代解读和章节摘要...\n');

// 现代解读和章节摘要翻译映射
const interpretationSummaryMap = {
  // 甲乙经相关
...
// 修复现代解读和章节摘要
function fixInterpretationSummary(bookId, bookName) {
    ...
    try {
      ...
      if (chapterData.sections) {
        chapterData.sections.forEach((section, index) => {
          // 修复现代解读
          if (section.interpretation && interpretationSummaryMap[section.interpretation]) {
            const originalInterpretation = section.interpretation;
            section.interpretation = interpretationSummaryMap[section.interpretation];
            hasChanges = true;
          ...
          // 修复章节摘要
          if (section.summary && interpretationSummaryMap[section.summary]) {
            const originalSummary = section.summary;
            section.summary = interpretationSummaryMap[section.summary];
            hasChanges = true;
...
books.forEach(book => {
  console.log(`\n📚 修复 ${book.name} 现代解读和摘要：`);
  const fixes = fixInterpretationSummary(book.id, book.name);
  totalFixes += fixes;
});
