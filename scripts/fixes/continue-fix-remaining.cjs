...
// 📅 Marked obsolete on: 2026-03-10T18:57:28.274Z
// 🔄 Purpose: Internationalization fixes (completed)
// 
const fs = require('fs');
const path = require('path');

console.log('🔧 继续修复剩余的数据问题...\n');

// 扩展的翻译映射
const extendedTranslationMap = {
  // 甲乙经剩余问题
  ...
  'Cold Damage': '伤寒',
  'Shaoyang Patterns': '少阳证型',
  'Taiyin Patterns': '太阴证型',
...
// 修复特定书籍的剩余问题
function fixRemainingIssues(bookId, bookName) {
    ...
    try {
      ...
      if (chapterData.sections) {
        chapterData.sections.forEach((section, index) => {
          // 修复现代解读
          if (section.interpretation && extendedTranslationMap[section.interpretation]) {
            const originalInterpretation = section.interpretation;
            section.interpretation = extendedTranslationMap[section.interpretation];
            hasChanges = true;
          ...
          // 修复白话译文
          if (section.translation && extendedTranslationMap[section.translation]) {
            const originalTranslation = section.translation;
            section.translation = extendedTranslationMap[section.translation];
            hasChanges = true;
          ...
          // 修复章节摘要
          if (section.summary && extendedTranslationMap[section.summary]) {
            const originalSummary = section.summary;
            section.summary = extendedTranslationMap[section.summary];
            hasChanges = true;
          ...
          if (section.keyConcepts) {
            section.keyConcepts.forEach(concept => {
              if (concept.term && extendedTranslationMap[concept.term]) {
                const originalTerm = concept.term;
                concept.term = extendedTranslationMap[concept.term];
                hasChanges = true;
              ...
              if (concept.category && extendedTranslationMap[concept.category]) {
                const originalCategory = concept.category;
                concept.category = extendedTranslationMap[concept.category];
                hasChanges = true;
              ...
              if (concept.relatedConcepts) {
                ...
                concept.relatedConcepts = concept.relatedConcepts.map(related => 
                  extendedTranslationMap[related] || related
                );
...
books.forEach(book => {
  console.log(`\n📚 修复 ${book.name} 剩余问题：`);
  const fixes = fixRemainingIssues(book.id, book.name);
  totalFixes += fixes;
});
