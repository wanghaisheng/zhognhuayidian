...
// 📅 Marked obsolete on: 2026-03-10T18:57:28.320Z
// 🔄 Purpose: Internationalization fixes (completed)
// 
const fs = require('fs');
const path = require('path');

console.log('🚀 修复所有剩余白话译文...\n');

// 完整的白话译文翻译映射
const allTranslationMap = {
  // 甲乙经相关
...
// 修复所有书籍的白话译文
function fixAllTranslation(bookId, bookName) {
    ...
    try {
      ...
      if (chapterData.sections) {
        chapterData.sections.forEach((section, index) => {
          // 修复白话译文
          if (section.translation && allTranslationMap[section.translation]) {
            const originalTranslation = section.translation;
            section.translation = allTranslationMap[section.translation];
            hasChanges = true;
...
books.forEach(book => {
  console.log(`\n📚 修复 ${book.name} 白话译文：`);
  const fixes = fixAllTranslation(book.id, book.name);
  totalFixes += fixes;
});
