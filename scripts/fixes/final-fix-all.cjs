...
// 📅 Marked obsolete on: 2026-03-10T18:57:28.285Z
// 🔄 Purpose: Internationalization fixes (completed)
// 
const fs = require('fs');
const path = require('path');

console.log('🚀 最终修复所有剩余数据问题...\n');
...
const finalTranslationMap = {
  ...
  'Acupuncture Study Chapter One': '针灸学第一章',
  ...
  'Zang-Fu Diseases Chapter One': '脏腑病第一章',
  ...
  'Wei-Qi-Ying-Xue Theory Chapter One': '卫气营血理论第一章',
  ...
  'Medical Origin Chapter One': '医学源流第一章',
...
// 修复特定书籍的最终问题
function finalFixBookData(bookId, bookName) {
    ...
    try {
      ...
      if (chapterData.sections) {
        chapterData.sections.forEach((section, index) => {
          // 修复现代解读
          if (section.interpretation && finalTranslationMap[section.interpretation]) {
            const originalInterpretation = section.interpretation;
            section.interpretation = finalTranslationMap[section.interpretation];
            hasChanges = true;
          ...
          // 修复白话译文
          if (section.translation && finalTranslationMap[section.translation]) {
            const originalTranslation = section.translation;
            section.translation = finalTranslationMap[section.translation];
            hasChanges = true;
          ...
          // 修复章节摘要
          if (section.summary && finalTranslationMap[section.summary]) {
            const originalSummary = section.summary;
            section.summary = finalTranslationMap[section.summary];
            hasChanges = true;
          ...
          // 修复章节标题
          if (section.title && section.title.zh && finalTranslationMap[section.title.zh]) {
            const originalTitle = section.title.zh;
            section.title.zh = finalTranslationMap[section.title.zh];
            hasChanges = true;
          ...
          if (section.keyConcepts) {
            section.keyConcepts.forEach(concept => {
              if (concept.term && finalTranslationMap[concept.term]) {
                const originalTerm = concept.term;
                concept.term = finalTranslationMap[concept.term];
                hasChanges = true;
              ...
              if (concept.category && finalTranslationMap[concept.category]) {
                const originalCategory = concept.category;
                concept.category = finalTranslationMap[concept.category];
                hasChanges = true;
              ...
              if (concept.relatedConcepts) {
                ...
                concept.relatedConcepts = concept.relatedConcepts.map(related => 
                  finalTranslationMap[related] || related
                );
...
books.forEach(book => {
  console.log(`\n📚 最终修复 ${book.name}：`);
  const fixes = finalFixBookData(book.id, book.name);
  totalFixes += fixes;
});
