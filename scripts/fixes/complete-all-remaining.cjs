...
// 📅 Marked obsolete on: 2026-03-10T18:57:28.264Z
...
// 最完整的翻译映射
const completeAllTranslationMap = {
  // 甲乙经剩余问题
...
// 完成修复特定书籍
function completeAllFixBookData(bookId, bookName) {
    ...
    try {
      ...
      if (chapterData.sections) {
        chapterData.sections.forEach((section, index) => {
          // 修复现代解读
          if (section.interpretation && completeAllTranslationMap[section.interpretation]) {
            const originalInterpretation = section.interpretation;
            section.interpretation = completeAllTranslationMap[section.interpretation];
            hasChanges = true;
          ...
          // 修复白话译文
          if (section.translation && completeAllTranslationMap[section.translation]) {
            const originalTranslation = section.translation;
            section.translation = completeAllTranslationMap[section.translation];
            hasChanges = true;
          ...
          // 修复章节摘要
          if (section.summary && completeAllTranslationMap[section.summary]) {
            const originalSummary = section.summary;
            section.summary = completeAllTranslationMap[section.summary];
            hasChanges = true;
          ...
          if (section.keyConcepts) {
            section.keyConcepts.forEach(concept => {
              if (concept.term && completeAllTranslationMap[concept.term]) {
                const originalTerm = concept.term;
                concept.term = completeAllTranslationMap[concept.term];
                hasChanges = true;
              ...
              if (concept.relatedConcepts) {
                ...
                concept.relatedConcepts = concept.relatedConcepts.map(related => 
                  completeAllTranslationMap[related] || related
                );
...
books.forEach(book => {
  console.log(`\n📚 完成修复 ${book.name}：`);
  const fixes = completeAllFixBookData(book.id, book.name);
  totalFixes += fixes;
});
