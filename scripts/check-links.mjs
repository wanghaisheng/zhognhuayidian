 ...
 * Output: A console report detailing broken links and accessibility issues (e.g., missing alt tags).
...
const extractLocalesFromLanguageConfig = (tsSource) => {
  ...
  const defaultLocale = defaultMatch ? String(defaultMatch[1]).trim() : 'en-US';
...
function resolveReportPath() {
  ...
  return path.join(DIST_DIR, 'audit-report.md');
}
...
function extractSourceVisibleText(html) {
  try {
    const dom = new JSDOM(html);
    const doc = dom.window.document;
    const nodes = doc.querySelectorAll('script, style, noscript');
...
function scanDistForClientSignals(distDir) {
  ...
  walkDir(distDir, filePath => {
    ...
    if (!content) return;
    if (!jsSignals.navigatorLanguage && content.includes('navigator.language')) jsSignals.navigatorLanguage = true;
    if (!jsSignals.localStorageLng && content.includes('localStorage') && content.includes('lng')) jsSignals.localStorageLng = true;
    if (!jsSignals.locationHash && (content.includes('location.hash') || content.includes('window.location.hash'))) jsSignals.locationHash = true;
...
async function runRedirectAndDuplicateAudit(baseUrl, siteConfig) {
  ...
  for (const s of samples) {
    ...
    const currentLang = s.finalPath.split('/')[1] || '';
    const currentLocale = locales.includes(currentLang) ? currentLang : siteConfig?.defaultLocale || 'en';
    const currentHreflang = (HREFLANG_MAP[currentLocale] || currentLocale).toLowerCase();
    
    if (currentLocale) {
      const selfAlt = (s.alternates || []).find(a => a.hreflang === currentHreflang);
      if (selfAlt) {
        try {
    ...
    if (currentLocale) {
      for (const alt of s.alternates || []) {
        ...
        if (alt.hreflang === currentLocale) continue;
        ...
        const back = (targetSample.alternates || []).find(a => a.hreflang === currentHreflang);
        if (!back) {
  ...
  if (samples.length > 0) {
    ...
    const canonicalStrategy = anyMissing ? '缺失' : anyNonSelf ? '非自引用' : '自引用';
...
function collectHtmlFiles(distDir) {
  ...
  walkDir(distDir, filePath => {
    ...
    if (/\/__debug_heads__\//.test(normalized) || normalized.endsWith('.head.html')) return;
...
async function checkLinks() {
  ...
  for (const filePath of htmlFiles) {
    ...
    const sourceText = extractSourceVisibleText(html);
  ...
  if (jsSignals.locationHash || jsSignals.createHashHistory) {
    issues.warn.push('[Routing] 构建产物包含 hash 路由/Location hash 的迹象（可能导致 URL 出现 #）');
  }
  if (jsSignals.navigatorLanguage || jsSignals.localStorageLng) {
    issues.warn.push('[i18n] 构建产物包含基于浏览器语言/localStorage 的跳转线索');
  }
