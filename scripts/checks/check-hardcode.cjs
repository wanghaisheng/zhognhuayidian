...
function scanFile(fp) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Detect Chinese text in quoted strings (object literals, function calls)
    if (ext === '.tsx' && /[\u4e00-\u9fff]/.test(line)) {
      const trimmed = line.trim();
      const isComment = trimmed.startsWith('//') || trimmed.startsWith('/*') || trimmed.startsWith('*');
      const isImport = /^\s*import\s/.test(trimmed);
      // Check if it's a string literal that likely contains UI text
      const hasChineseInQuotes = /['"`][^'"`]*[\u4e00-\u9fff][^'"`]*['"`]/.test(line);
      const isObjectProperty = /[a-zA-Z_$][a-zA-Z0-9_$]*\s*:\s*['"`]/.test(line);
      const isFunctionCall = /t\(['"`][^'"`]*[\u4e00-\u9fff][^'"`]*['"`]\)/.test(line);
      
      if (!isComment && !isImport && (hasChineseInQuotes && (isObjectProperty || !isFunctionCall))) {
        findings.push({ key: 'zh_string_literal', line: i + 1, snippet: trimmed.slice(0, 160) });
      }
    }
